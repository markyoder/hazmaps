...
class hazCrack(object):
	...
	def __init__(self, catalog=None, catnum=0, mc=2.5, gridsize=.5, ndithers=3, winlen=128, avlen=1, bigmag=5.0, fignum=0, logZ=1.0):
		...
		#self.logZ=1.0
		#self.forecastDate=dtm.datetime.now()	# or we might change this.
		self.forecastDate=dtm.datetime(2011, 3,5)
		self.hmdate=dtm.datetime.now()
		...
		for i in xrange(ndithers*ndithers):
			...
			thishm=rbHazMapLean(catalog=catalog, catnum=catnum, mc=mc, gridsize=gridsize, phi=[yphase, xphase], winlen=winlen, avlen=avlen, bigmag=bigmag, logZ=self.logZ) # but we'll need to suppress fig-plotting and i don't know if the phases are switched.
	...
	def boxesTo(self, hmdate=dtm.datetime.now(), fignum=0):
		self.hazMapTo(hmdate)
		return self.simpleBoxes(fignum)
	
	def hazMapTo(self, hmdate=dtm.datetime.now()):
		self.hmdate=hmdate
	...
	def contourMap(self, fignum=1, nconts=12, mapres='i', maptransform=True):
		# maptransform: transform to matplotlig.basemap coords. for KML export, set this to false (coords in LL).
		...
		plt.title("Record-breaking contour hazard map\ndtm=%s, $d\\lambda=%.4f$, nDith=%d, mc=%.2f, rblen=%d, avlen=%d\n\n" % (str(catdate), self.gridsize, self.ndithers, self.mc, self.winlen, self.avlen))
		...
		plt.title("Record-breaking contour haz-map\ndtm=%s, $d\\lambda=%.4f$, nDith=%d, mc=%.2f, rblen=%d\n\n" % (str(catdate), self.gridsize, self.ndithers, self.mc, self.winlen))
		#
		llrange=self.hazmaps[0].catalog.getLatLonRange(self.hazmaps[0].thiscat)
		deltaLat=abs(float(self.llrange[1][0])-self.llrange[0][0])
		deltaLon=abs(float(self.llrange[1][1])-self.llrange[0][1])
		...
		plt.figure(fignum)
		#plt.clf()
		self.conts = plt.contourf(X,Y,Z2d, nconts, alpha=.85, zorder=10)
		plt.colorbar()
		plt.spectral()
		
		plt.figure(fignum+1)
		plt.contourf(X1,Y1,Z2d, nconts, alpha=.85, zorder=10)
		plt.colorbar()
		plt.spectral()
	...
	def makeColorbar(self, cset=None, colorbarname=None, reffMag=5.0, contfact=5.0):
		...
		cb1.set_label('Local RB ratio, $r=\\log (n_{rb-short}/n_{rb-long})$')
	...
	def simpleBoxes(self, fignum=0, thresh=0.0, plotevents=1, mapres='l'):
		# do a simple matplotlib plot of hazmap. this is probably diagnostic, so it may just be points of some sort.
		# note: this plots the ll-corner of each box. for a science-interesting plot, we should center these points (aka, [x+L/2, y+L/2]
...
class rbHazMapLean(object):
	...
	def hazMapTo(self, forecastDate=None, mev=None):
		...
		for rw in self.catalog.getcat(0):
			...
			#print rw[0], forecastDate
			if mpd.date2num(rw[0])<mpd.date2num(forecastDate): precat+=[rw]
			if mpd.date2num(rw[0])>=mpd.date2num(forecastDate): postcat+=[rw]
		#
...
def crackMap(objCat=None, catnum=0, mc=2.5, gridsize=None, ndithers=4, winlen=20, sigma=1.68, avlen=None, todt=dtm.datetime(2010, 4, 1), epicen=None, thresh=0):
	...
	#
	objHM=hazCrack(objCat, catnum, mc, gridsize, ndithers, winlen, avlen, bigmag, fignum)
	#
	#objHM.epicen=epicen
	objHM.hazMapTo(todt)
	#
